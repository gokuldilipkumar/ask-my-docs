...
## 2026-07-17 — Auditing Block 8: a fresh-eyes review's blind spot cuts both ways

### Dismissing a finding needs the same evidence bar as accepting one
The fresh-eyes subagent's top "Blocking" finding on this audit — "no ingest step, index probably not committed, both CI gates should fail on a clean checkout" — was completely wrong, and wrong for an understandable reason: it was given only a fixed file list (source, tests, workflow YAML), not the repo's actual tracked-file state, so it had no way to know `data/index/` was genuinely committed. The temptation with a wrong-looking finding is to just say "that's not right" and move on, but the only thing that actually justifies dismissing it here is the same kind of evidence that would justify fixing it: `git ls-files data/` showing all 13 index files tracked, plus two independent real GitHub Actions runs that had already used exactly that index successfully. A fresh-eyes review's power comes from having zero stake in the story already told (this project's own Block 6/7 lesson) — but zero stake also means zero access to context the reviewer never had, and a finding built on a false premise needs to be verified false with the same rigor a real finding gets verified true, not waved away because it "seems obviously fine" to whoever already knows the missing context.

### A concurrency gap doesn't announce itself until two runs actually overlap
`cheap-gate.yml` triggering on `[push, pull_request]` with no `permissions:` block and no `concurrency` group was never going to fail a test, break a build, or show up in any log this project had produced so far — because this repo has never had two commits land close enough together, or a pull request opened, for the double-run risk to actually manifest. Nothing about the code was broken; the gap was in a property of the *system* (two workflow runs racing) that the system had simply never yet been in a position to exercise. This is a different shape of bug than most of what this project's audits have caught before (a wrong value, a missed branch, an unwired parameter) — it's a correctness property that's true by luck-of-timing so far, not by design, and the only way to catch it is to explicitly ask "what happens if two of these overlap" rather than wait for it to happen.
