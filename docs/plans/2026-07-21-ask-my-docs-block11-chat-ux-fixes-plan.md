...
- [ ] **Not achieved, not abandoned**: two real full `eval` runs (`top_n=10` and, diagnostically, `top_n=20`, both `answer_v2`) gave non-monotonic `correctness_rate`/`completeness_rate` swings with no clean single-track cause — `completeness_rate` failed gate in both configs. Not confirmed as a real regression (small 8-question golden set, real LLM-judge stochasticity); logged to `BUGS.md` rather than chased with further real-cost eval runs. No baseline saved this block.
- [x] A live-API test confirms `answer_text` from `answer_v2` contains no `[hexid]`-shaped bracket patterns for a real in-scope question, while `GeneratedAnswer.citations` is still populated with real chunk ids. Real result (2026-07-22): held on the first attempt, no iteration needed.
- [x] `uv run pytest -m "not slow"` and the full suite stay green throughout, including the six updated `_get_model` fakes in `tests/rerank/test_cross_encoder.py` (the plan's own audit missed a 6th call site, found and fixed during `/build`).
- [~] `config.yaml`/`config.example.yaml` stay in sync (`rerank.max_length` present in both, unchanged this session) — done. `BUGS.md` updated with the eval-noise finding — done. `.agent/decisions.log`/README updates (Chunk 11.7) — **not done**, deferred to a future session alongside a decision on the unresolved eval-gate finding above.
