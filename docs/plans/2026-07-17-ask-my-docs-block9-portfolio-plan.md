...
- [ ] A real Langfuse Cloud trace exists for a real query against the real corpus, screenshotted and embedded in the README's observability section; `tests/observability/test_live_trace.py` (Block 7's deferred Chunk 7.10) passes with `RUN_LIVE_LANGFUSE_TESTS=1`.
- [ ] A real, reproducible regression run (`rerank.enabled: false`) shows a measured drop in `recall_at_k`/`mrr`/`ndcg` vs. the real committed baseline, captured via the actual `eval --retrieval-only` CLI output (including its PASS/FAIL gate table), with `config.yaml` reverted afterward.
- [ ] `docs/eval-report.md` exists: baseline metrics table (both stored baselines), the regression example with real before/after numbers and real CLI output, and a short "what this harness has already caught" section referencing the real q4 fabrication bug and `mrr` vacuous-truth fix from project history.
- [ ] `README.md` exists at repo root: project pitch/audience framing, a Mermaid architecture diagram matching the real pipeline (verified against real span names and real module call chains), an explained rationale for hybrid retrieval + RRF fusion + reranking + citation verification (the *why*, not just the *what*), a quickstart section where every shown command was actually run and its real output captured, an observability section with the real trace screenshot, a link to `docs/eval-report.md`, and CI status badges for both workflows.
- [ ] Every command shown in `README.md` was actually executed this session against the real repo state and its output verified to match what's documented (no hand-typed illustrative output).
- [ ] `git diff config.yaml` shows no net change after this block (the regression chunk's edit is fully reverted).
