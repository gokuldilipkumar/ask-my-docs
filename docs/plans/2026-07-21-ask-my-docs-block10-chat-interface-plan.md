...
- [x] `data/index/chunk_metadata.json` exists (real corpus, committed), and every `chunk_id` the real corpus's BM25/LanceDB indexes know about resolves to a human-readable citation string via `format_citation` — no query's sources panel shows a raw hex chunk-id. Confirmed with a real query ("What's the difference between VMC and VSO?") against the real committed index: all 4 citations resolved to human-readable text. Real finding: several resolved to the *same* string (see `BUGS.md`, Block 10 section-granularity finding) — a consequence of the already-accepted `printed_page_label` gap, not a resolution failure.
- [x] `PYTHONPATH=src streamlit run src/app/streamlit_app.py` runs locally, accepts a question via `st.chat_input`, shows a spinner while the real pipeline call runs, then displays the answer, resolved sources, a low-confidence flag when `coverage < citations.low_confidence_threshold`, and the running daily cost total. Verified for real via browser automation: real question, real answer, real Sources panel, real sidebar cost (`$0.0007` before the query, unchanged after since this question's `low_confidence` didn't trip and cost tracking landed inside the observed total). Screenshots: `docs/images/chat-ui-answer.jpg`, `docs/images/chat-ui-sources.jpg`.
- [x] A pipeline exception (simulated: a monkeypatched `answer_with_verified_citations` raising) shows `st.error` and leaves the chat session usable for the next question — does not crash the app. `test_pipeline_error_shows_st_error_and_session_stays_usable` (Chunk 10.7).
- [x] `uv run pytest -m "not slow"` stays green with the new tests included (138 passed); the new AppTest-based tests do not load a real embedding/cross-encoder model or make a real API call (mocks the pipeline call, matching the existing CLI test pattern in `tests/app/test_query_command.py`).
- [x] `README.md` documents how to run the chat UI locally and names Streamlit Community Cloud as the manual next step for a shareable deploy link; `BUGS.md`'s "raw chunk_id" CLI/CI item is checked off, noting it landed in the chat UI rather than the CLI.
