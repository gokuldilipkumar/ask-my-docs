...
2. **Third-Party API Verification**
   ...
   - **Performance claims need production-shaped probe inputs.** A latency/throughput probe on toy inputs can "confirm" a design claim that is false for real data: the reranker probed at 63ms/16 pairs on short strings but runs ~5.3s/20 candidates on real 329-token-median chunks — an 80x gap the probe hid. When the claim under verification is about speed or size, probe with real corpus samples (real lengths), not fixture-sized strings.
...
3. **Contract & Isolation Checks**
   ...
   - **Untested defenses are bugs waiting.** Do not add defensive code (limits, clamps, fallbacks) that no probe evidence demands and no RED test exercises — especially not against a threat a probe just showed doesn't exist. A `.limit(len(ids))` added as "cheap insurance" *after* the probe showed no default cap became itself the bug: duplicate rows inflated the match count past the limit and silently truncated the scan. Every defense gets its own RED test proving it defends, or it stays out.
