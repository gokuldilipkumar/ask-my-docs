...
*   **Additive-Parameter Reach Check**: When a phase adds a new optional parameter to an already-shipped function to extend a cross-cutting capability (observability, auth, feature flags), grep every existing call site of that function — not just the ones this phase's own commits touched — and verify each one that should benefit from the new capability actually passes a real value, not the safe default. A safe default makes the gap invisible to "tests still pass": Block 7 added `observability`/`observability_config` params to 5 functions, but `eval/pipeline.py`'s `_evaluate_one` (an existing caller of 4 of them) was never updated, so real eval-run API costs silently never counted toward the daily budget cap — caught only by a fresh-eyes review, not by any test.
...
*   **Spy Assertion Check**: For any test double that records call arguments into a list/attribute (`.update_calls`, `.calls`, a spy's `self.spans`), verify the test actually asserts on that captured payload's *contents*, not just that the method was invoked. Two Block 7 tests captured `span.update(usage_details=..., cost_details=...)` into `update_calls` and never once asserted on it — the cost/usage values attached to a real Langfuse span were untested despite looking covered.
...
*   **CI Workflow Correctness**: For any new or changed `.github/workflows/*.yml` this phase, apply the same scrutiny as Python code, not "looks like standard CI boilerplate" — check for an explicit `permissions:` block (don't rely on the repo default), a `concurrency` group sized to the trigger (`cancel-in-progress: true` for push/PR gates that re-run cheaply; `false` for jobs with real API cost, so an in-progress run finishes instead of being killed mid-spend), and that every job needing a built artifact or secret has a verified real path to get it — confirmed by an actual `gh run` on GitHub, not by reading the YAML. Block 8's fresh-eyes review found `cheap-gate.yml` missing both `permissions` and a `concurrency` group despite triggering on `[push, pull_request]`.
*   **Trace Nesting Reach Check**: If tracing/observability spans were added or changed this phase, verify every independent orchestrator that opens multiple leaf spans (retrieval, generation, verification, etc.) wraps its own call chain in one enclosing span — a shared tracer/client *instance* across leaf calls is necessary but not sufficient for real nesting (most tracing SDKs nest a child under whatever parent context is currently active, not under "whatever else used the same client"). Unit tests asserting span names/counts on a fake tracer cannot catch a missing wrapping span; this needs either a real backend check or a structural enter/exit-order assertion. Block 7 shipped full span coverage with every leaf function tested, but no orchestrator ever opened a wrapping span — invisible until a real trace screenshot one block later showed 6 separate root traces instead of one nested trace.
