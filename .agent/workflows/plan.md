...
7. **Conventions Check**: Before writing the plan, read 1-2 existing files in each category you'll be creating (test, component, API route). Scan `.agent/gotchas.md` for patterns relevant to this work. Document key conventions as constraints in the plan:
     ...
     - [ ] **Composition Cost Audit**: If the plan composes an existing higher-level function (e.g. an already-shipped orchestrator like `answer_question`) alongside code that independently needs one of that function's expensive upstream steps (retrieval, rerank, generation), check whether the inputs are already computed earlier in the same task and can be threaded through instead of recomputed. A cheap duplicate (a second small lookup) can be accepted as debt; an expensive one (a multi-second retrieval/rerank pass) cannot — state which case applies and why. Block 6's eval harness independently retrieved+reranked to score metrics, then called `answer_with_verified_citations`, which retrieved+reranked *again* internally — invisible in mocked tests, ~5.3s/query real cost, caught only at `/audit`.
     - [ ] **Additive-Parameter Reach Audit**: If the plan adds a new optional parameter to already-shipped functions to extend a cross-cutting capability (observability, auth, caching), list every existing real caller of each touched function and state explicitly whether it needs updating to pass the new capability — "additive-optional, existing call sites keep working" is true but insufficient if one of those existing call sites should have opted in and silently didn't. Block 7's plan never listed `eval/pipeline.py` as a caller needing an update, so its real API costs went untracked until caught at `/audit`.
     ...
     - [ ] **Predicted-Behavior Claim Check**: If a plan chunk's chosen example/scenario depends on predicting how a real system will behave under a specific change (e.g. "disabling X should degrade metric Y, demonstrating Z"), that prediction is an empirical claim, not a design fact — flag it as unverified in the plan text rather than writing the expected outcome as if confirmed. A regression demo planned around "disabling reranking will measurably hurt retrieval metrics" turned out backwards in reality (it improved every metric) — the real, gate-failing regression needed two more real attempts at build time to find. Where cheap to do at plan time (a single real command run), verify the prediction before committing to it as the plan's specific chosen scenario.
